...
def ejecutar_modelo_completo(params):
    ...
    # Definición de corriente de entrada con precio
    ...
    # Asignar precio al producto para que BioSTEAM calcule los ingresos (Sales)
    V100.outs[0].price = params['p_etanol']
    
    # Configuración de TEA con parámetros obligatorios
    tea = bst.TEA(
        system=sys,
        IRR=0.15,                           
        duration=(2026, 2046),              
        depreciation='MACRS7',              # Requerido por versión actual
        construction_schedule=(0.5, 0.5),   
        startup_months=3,                   
        operating_days=330,                 
        income_tax=0.30,                    
        lang_factor=4.0,                    # Factor de inversión
        startup_FOCfrac=0.5,                
        startup_VOCfrac=0.5,                
...
if ejecutar:
    ...
    try:
        ...
        # SOLUCIÓN AL ERROR _FOC: Usar atributos públicos y verificados
        with e1: st.metric("NPV (VPN)", f"${tea.NPV/1e6:.2f} M")
        with e2: 
            # ROI = (Ventas Anuales - Costos Operativos Anuales) / Inversión Total
            roi_val = (tea.sales - tea.AOC) / tea.TCI * 100 if tea.TCI != 0 else 0
            st.metric("ROI", f"{roi_val:.1f}%")
        with e3:
            # Payback = Inversión / (Ingresos Netos)
        ...
        with t2:
            if ia_tutor and "GEMINI_API_KEY" in st.secrets:
                ...
                prompt = f"Como experto en ingeniería, analiza: NPV=${tea.NPV/1e6:.2f}M y ROI={roi_val:.1f}%."
