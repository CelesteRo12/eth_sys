...
# =================================================================
st.set_page_config(page_title="Simulador Pro Etanol ISO", layout="wide")
...
st.markdown("""
    ...
    .metric-value { font-size: 1.3rem; font-weight: bold; color: #155724; }
    </style>
""", unsafe_allow_html=True)

# =================================================================
# 2. LÓGICA DE INGENIERÍA (Blindada contra TypeErrors)
# =================================================================
def simular_sistema_seguro(t_f, t_w, p_v, p_l, p_vap, p_h, p_m, p_e):
    ...
    # Configurar precio de electricidad
    ...
    # Usamos una lista de comprensión con un filtro if para evitar NoneType
    costos_u = [u.utility_cost for u in sys.units if u.utility_cost is not None]
    costo_op_total = (sys.power_utility.cost + sum(costos_u)) * 8000
    
    inv_cap = 190000
    ingresos = W310.F_mass * p_e * 8000
    materia_prima = mosto.F_mass * p_m * 8000
    ...
    return sys, W310, npv, pb, roi, neto
...
with st.sidebar:
    ...
    st.divider()
    tutor_on = st.toggle("🎓 Modo Tutor IA")
    btn = st.button("🚀 EJECUTAR PROCESO", use_container_width=True)

# =================================================================
# 4. RESULTADOS
# =================================================================
if btn:
    sys, prod, npv, pb, roi, neto = simular_sistema_seguro(v_tf, v_tw, v_pv, v_pl, v_pvap, v_ph, v_pm, v_pe)

    # 4.1 Recuadros de Producto Final
    st.subheader("📋 Parámetros de Salida")
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.markdown(f"<div class='metric-card'>Presión<br><div class='metric-value'>{prod.P/101325:.2f} atm</div></div>", unsafe_allow_html=True)
    with c2: st.markdown(f"<div class='metric-card'>Temperatura<br><div class='metric-value'>{prod.T-273.15:.1f} °C</div></div>", unsafe_allow_html=True)
    with c3: st.markdown(f"<div class='metric-card'>Flujo<br><div class='metric-value'>{prod.F_mass:.1f} kg/h</div></div>", unsafe_allow_html=True)
    eth_p = (prod.imass['Ethanol']/prod.F_mass)*100 if prod.F_mass > 0 else 0
    with c4: st.markdown(f"<div class='metric-card'>Comp. Etanol<br><div class='metric-value'>{eth_p:.1f} %</div></div>", unsafe_allow_html=True)

    # 4.2 Indicadores Económicos
    st.divider()
    st.subheader("💰 Análisis de Rentabilidad")
    e1, e2, e3, e4 = st.columns(4)
    e1.metric("Costo Real Prod.", f"{v_pm * 1.25:.3f} USD/kg")
    e2.metric("NPV", f"{npv/1000:.1f}k USD")
    ...
    with t_en:
        st.table(pd.DataFrame([{"Unit": u.ID, "kW": round(sum([h.duty for h in u.heat_utilities])/3600, 2)} for u in sys.units]))

    # 4.4 Tutor IA
    if tutor_on:
        st.divider()
        st.subheader("💬 Ventana de Diálogo con IA")
        if "chat" not in st.session_state: st.session_state.chat = []
        
        for m in st.session_state.chat:
            with st.chat_message(m["role"]): st.markdown(m["content"])
            
        if prompt := st.chat_input("Consulta al experto..."):
            ...
            if "GEMINI_API_KEY" in st.secrets:
                ...
                res = model.generate_content(f"Planta Etanol. ROI: {roi:.1f}%. Pregunta: {prompt}")
                with st.chat_message("assistant"): st.markdown(res.text)
                st.session_state.chat.append({"role": "assistant", "content": res.text})
            else:
                st.warning("Configura GEMINI_API_KEY en Secrets.")
else:
    st.info("Ajuste los valores y presione el botón de ejecución.")
